...
func GetFeedStreamWithLive(httpCtx *gin.Context) {
	req := &live_feed_model.GetFeedStreamWithLiveReq{}
	...
	resp, err := live_feed_service.GetFeedStreamWithLive(httpCtx, req.Offset, req.PageSize, req.UserID)
	if err != nil {
		logs.CtxError(httpCtx, "GetFeedStreamWithLive err: %s", err)
