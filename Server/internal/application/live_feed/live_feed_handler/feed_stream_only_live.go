...
func GetFeedStreamOnlyLive(httpCtx *gin.Context) {
	req := &live_feed_model.GetFeedStreamOnlyLiveReq{}
	...
	resp, err := live_feed_service.GetFeedStreamOnlyLive(httpCtx, req.Offset, req.PageSize, req.RoomID, req.UserID)
	if err != nil {
		logs.CtxError(httpCtx, "GetFeedStreamOnlyLive err: %s", err)
