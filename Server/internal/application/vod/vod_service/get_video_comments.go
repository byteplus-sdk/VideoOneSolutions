...
func GetVideoComments(ctx context.Context, vid string) ([]*vod_models.Comments, error) {
	repo := vod_repo.GetVodRepo()
	videos, err := repo.GetCommentsByVid(ctx, vid)
	if err != nil {
	...
	var res = make([]*vod_models.Comments, 0)
	for _, v := range videos {
		res = append(res, &vod_models.Comments{
			Content:    v.Content,
			Name:       v.Name,
