...
func (uf *UserFactory) NewUser(ctx context.Context, appID, roomID, userID, userName, deviceID string, userRole int) *User {
	dbUser := &ktv_entity.KtvUser{
		AppID:          appID,
		RoomID:         roomID,
		UserID:         userID,
		UserName:       userName,
		UserRole:       userRole,
		NetStatus:      ktv_db.UserNetStatusOffline,
		InteractStatus: ktv_db.UserInteractStatusNormal,
		SeatID:         0,
		Mic:            1,
		CreateTime:     time.Now(),
		UpdateTime:     time.Now(),
...
func (uf *UserFactory) Save(ctx context.Context, user *User) error {
	if user.IsDirty() {
		...
		err := uf.userRepo.Save(ctx, user.KtvUser)
		if err != nil {
