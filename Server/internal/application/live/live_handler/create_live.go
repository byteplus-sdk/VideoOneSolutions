...
func CreateLive(ctx *gin.Context) (resp interface{}, err error) {
	...
	room, host, err := roomService.CreateRoom(ctx, p.AppID, p.RoomName, p.UserID, p.UserName)
	if err != nil {
	...
	resp = &createLiveResp{
		...
		RtmToken:      live_util.GenToken(room.RoomID, p.UserID),
		RtcToken:      live_util.GenToken(roomRepo.GetRoomRtcRoomID(ctx, room.RoomID), p.UserID),
