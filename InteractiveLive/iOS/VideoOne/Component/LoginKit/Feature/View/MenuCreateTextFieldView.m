...
@interface MenuCreateTextFieldViewLogin ()
...
@implementation MenuCreateTextFieldViewLogin
...
- (void)cancelButtonAction {
    ...
    if (self.minLimit > 0) {
        ...
        if([self.delegate respondsToSelector:@selector(showErrorLabelWithMessage:isShow:)]) {
            [self.delegate showErrorLabelWithMessage:message isShow:YES];
        }
...
//    self.errorLabel.hidden = YES;
    if([self.delegate respondsToSelector:@selector(showErrorLabelWithMessage:isShow:)]) {
        [self.delegate showErrorLabelWithMessage:@"" isShow:NO];
    }
}
...
- (UITextField *)textField {
    if (!_textField) {
        ...
        _textField.textColor = [UIColor colorFromHexString:@"#1D2129"];
