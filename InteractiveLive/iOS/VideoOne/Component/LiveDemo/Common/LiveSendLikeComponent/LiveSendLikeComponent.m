...
- (void)show {
    ...
    iconView.image = [UIImage imageNamed:randomIcon bundleName:HomeBundleName];
    ...
    CGFloat viewWidth = self.contentView.width;
    CGFloat viewHeight = self.contentView.height;
    NSInteger travelDirection = 1 - (2 * (int)arc4random_uniform(2));
    NSInteger m1 = travelDirection > 0
        ? viewX + (int)arc4random_uniform(viewWidth - viewX)
        : viewX - (int)arc4random_uniform(viewX);
    NSInteger n1 = viewY -  (viewHeight * 0.3) +  travelDirection * (int)arc4random_uniform(10);
    NSInteger m2 = travelDirection > 0
        ? viewX + (int)arc4random_uniform(viewWidth - viewX)
        : viewX - (int)arc4random_uniform(viewX);
