...
class AudioMediaMixingViewController: BaseViewController, ByteRTCEngineDelegate, ByteRTCRoomDelegate, ByteRTCMediaPlayerEventHandler {
    ...
    @objc func startAudioMix() {
        ...
        if "qilixiang.m4a" == self.audioTextField1.text {
            ...
            ToastComponents.shared.show(withMessage: getString(key: "wrongFormat"))
        }
...
//                endOffset = self.startOffset + size
    ...
    func createUI() -> Void {
        ...
        totalChannelLabel1.snp.makeConstraints { make in
            make.top.equalTo(audioPositionSetting.snp.bottom).offset(10)
            make.left.equalTo(audioPositionSetting)
        }
        
        fileBackgroundView.addSubview(audioTrackSetting)
        audioTrackSetting.snp.makeConstraints { make in
            make.top.equalTo(totalChannelLabel1.snp.bottom).offset(10)
            make.left.equalTo(totalChannelLabel1)
        }
        
        fileBackgroundView.addSubview(audioPitchSetting)
        audioPitchSetting.snp.makeConstraints { make in
            make.top.equalTo(audioTrackSetting.snp.bottom).offset(10)
            make.left.equalTo(audioTrackSetting)
        }
        ...
        audioSpeedSetting.snp.makeConstraints { make in
            ...
            make.left.equalTo(audioPitchSetting)
        }
        
        fileBackgroundView.addSubview(currentPostionLabel)
        currentPostionLabel.snp.makeConstraints { make in
            make.top.equalTo(audioSpeedSetting.snp.bottom).offset(10)
            make.left.equalTo(audioSpeedSetting)
        }
        
        fileBackgroundView.addSubview(currentProgressLabel)
        currentProgressLabel.snp.makeConstraints { make in
            make.top.equalTo(currentPostionLabel.snp.bottom).offset(10)
            make.left.equalTo(currentPostionLabel)
        }
        
        fileBackgroundView.addSubview(channelModeSheetView)
        channelModeSheetView.snp.makeConstraints { make in
            make.top.equalTo(currentProgressLabel.snp.bottom).offset(10)
            make.left.equalTo(currentProgressLabel)
        }
    ...
    lazy var audioMixLabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: "audioMixing")
        return label
    }()
    ...
    lazy var audioVolumeSetting: SliderView = {
        ...
        settingView.title = getString(key: "volume")
        return settingView
    }()
    
    lazy var audioPitchSetting: SettingItemView = {
        let settingView = SettingItemView()
        settingView.title = getString(key: "voicePitch")
    ...
    lazy var audioPositionSetting: SettingItemView = {
        let settingView = SettingItemView()
        settingView.title = getString(key: "seekPlayPosition")
        return settingView
    }()
    
    lazy var totalChannelLabel1: UILabel = {
        let label = UILabel()
        label.text = getString(key: "numOfTracks")
        return label
    }()
    
    lazy var audioTrackSetting: SettingItemView = {
        let settingView = SettingItemView()
        settingView.title = getString(key: "playSpecifiedTrack")
        return settingView
    }()
    
    lazy var audioSpeedSetting: SettingItemView = {
        let settingView = SettingItemView()
        settingView.title = getString(key: "playSpeed")
        return settingView
    }()
    
    lazy var channelModeSheetView: ActionSheetView = {
        let actionSheetView = ActionSheetView.init(title: getString(key: "channelMode"), optionArray: ["Auto","ModeL","ModeR","ModeMix"], defaultIndex: 0)
    ...
    lazy var currentPostionLabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: "currentPlaySpeed").appending(":")
        return label
    }()
    ...
    lazy var pcmMixLabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: "PCMMixing")
        return label
    }()
    
    lazy var pcmField: TextFieldView = {
        let field = TextFieldView()
        field.title = getString(key: "PCMFile").appending(":")
    ...
    func onMediaPlayerStateChanged(_ playerId: Int32, state: ByteRTCPlayerState, error: ByteRTCPlayerError) {
        ...
        if playerId == self.audioPlayerId && state == .playing {
            ...
            DispatchQueue.main.async {
                self.totalTimeLabel1.text = getString(key: "totalDuration").appending(": \(totalTime ?? 0)")
                self.totalChannelLabel1.text = getString(key: "numOfTracks").appending(": \(tracks ?? 0)")
            }
