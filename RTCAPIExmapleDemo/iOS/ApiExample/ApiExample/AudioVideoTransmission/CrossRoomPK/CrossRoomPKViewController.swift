...
class CrossRoomPKViewController: BaseViewController, ByteRTCEngineDelegate, ByteRTCRoomDelegate {
    ...
    @objc func joinRoom()  {
        ...
        if roomId == roomId2 {
            ToastComponents.shared.show(withMessage: getString(key: "roomidSameHint"))
            return
        }
    ...
    lazy var roomLabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: getString(key: "room"))
        return label
    }()
    ...
    lazy var forwardRoomLabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: "crossRoomForwarding")
        return label
    }()
    ...
    lazy var updateforwardButton: UIButton = {
        let button = BaseButton.init()
        button.setTitle(getString(key: "update"), for: .normal)
    ...
    func rtcRoom(_ rtcRoom: ByteRTCRoom, onForwardStreamStateChanged infos: [ByteRTCForwardStreamStateInfo]) {
        ToastComponents.shared.show(withMessage: "onForwardStreamStateChanged state: \(String(describing: infos.first?.state.rawValue))，error: \(String(describing: infos.first?.error.rawValue))")
    }
