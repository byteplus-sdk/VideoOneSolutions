...
class PullRTMPViewController: BaseViewController,VeLivePlayerObserver {
    ...
    @objc func startPull() {
        if let text = self.urlTextFieldView.text, !text.isEmpty {
            ToastComponents.shared.show(withMessage: getString(key: "curStreamingString").appending(":\(text)"))
            let newText = "\(PullUrlPre)\(text).flv"
            ToastComponents.shared.show(withMessage: getString(key: "curStreamingUrl").appending(":\(newText)"))
            self.livePlayer?.setPlayUrl(newText)
            self.livePlayer?.play()
            print("URL: \(newText)")

        } else {
            ToastComponents.shared.show(withMessage: getString(key: "invalidStreamingUrl"))
        }
    }
    ...
    lazy var urlTextFieldView: TextFieldView = {
        ...
        settingView.title = getString(key: "streamingUrl")
    ...
    lazy var receivedSEILabel: UILabel = {
        let label = UILabel()
        label.text = getString(key: "receivedSEI")
        return label
    }()
