...
public class MainActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.version)).setText(String.format("SDK Version: %s", RTCEngine.getSDKVersion()));
    }
            ...
            if (activityInfos != null) {
                for (ActivityInfo activityInfo : activityInfos) {
                    ...
                    if (activityClass.isAnnotationPresent(ApiExample.class)) {
                        ...
                        int category = apiExample.category();
                        String categoryName = category == -1 ? "" : getResources().getString(category);
                        int name = apiExample.name();
                        String nameStr = getResources().getString(name);
                        double order = apiExample.order();
                        ExampleInfo info = new ExampleInfo(nameStr, activityClass, order);
