...
public class KTVRoomListAdapter extends RecyclerView.Adapter<BVH<LayoutKtvRoomItemBinding>> {
    ...
    public void onBindViewHolder(@NonNull BVH<LayoutKtvRoomItemBinding> holder, int position) {
        ...
        holder.binding.userName.setText(mContext.getString(R.string.label_create_name_xxx, info.hostUserName));
        Glide.with(holder.binding.userAvatar)
                .load(Avatars.byUserId(info.hostUserId))
                .into(holder.binding.userAvatar);

        holder.binding.ktvAudienceCount.setText(formatAudienceCount(info.audienceCount + 1));
