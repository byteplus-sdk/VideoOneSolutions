...
class LLMChoiceFragment : Fragment(R.layout.fragment_ai_setting_recycler) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.title.setText(R.string.ai_setting_label_core_llm)

        val current = viewModel.llmModel.value?.value
        val items = viewModel.llms.map { LLMChoice(it, it.name == current) }

        binding.save.setOnClickListener {
            items.first { it.chosen }.let {
                if (it.text != current) {
                    viewModel.llmModel.postValue(Option(it.text, Source.USER))
                }
            }
    ...
    class LLMChoice(llm: LLM, override var chosen: Boolean) : IChoice {
        override val text: String = llm.name
        override val icon: String = llm.icon
