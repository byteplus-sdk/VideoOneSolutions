...
- (void)changeSubtitleAction:(id)param {
    if ([param isKindOfClass:[NSDictionary class]]) {
        NSDictionary *paramDic = (NSDictionary *)param;
        id value = paramDic.allValues.firstObject;
        if ([value isKindOfClass:[SubtitleidType class]]) {
            SubtitleidType *subtitle = (SubtitleidType *)value;
...
- (void)playerCoreReadyToPlay:(id<VEPlayCoreAbilityProtocol>)core {
    if (core == self.core) {
        [self.pipManager updatePlaybackStatus:YES];
    }
}
...
- (void)playerCore:(id<VEPlayCoreAbilityProtocol>)core playbackStateDidChanged:(VEPlaybackState)currentState info:(NSDictionary *)info {
    if (core == self.core) {
        ...
        [self.pipManager updatePlaybackStatus:currentState == VEPlaybackStatePlaying];
...
- (void)enablePIP:(void (^)(NSInteger))block {
    [[ToastComponent shareToastComponent] showLoadingWithMessage:LocalizedStringFromBundle(@"vod_pip_toast", @"VEVodApp")];
    ...
    [self autoEnablePIP:^(PIPManagerStatus status) {
        ...
        if (status == PIPManagerStatusStartSuccess) {
            [[ToastComponent shareToastComponent] showWithMessage:LocalizedStringFromBundle(@"vod_pip_toast_successfully", @"VEVodApp")];
        } else {
            wself.pipManager.enablePictureInPicture = NO;
            [[ToastComponent shareToastComponent] showWithMessage:LocalizedStringFromBundle(@"vod_pip_toast_failed", @"VEVodApp")];
        }
        if (block) {
            block(status);
        }
