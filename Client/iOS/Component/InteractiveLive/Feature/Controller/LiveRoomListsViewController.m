...
- (UILabel *)noHostLabel {
    if (!_noHostLabel) {
        ...
        _noHostLabel.text = LocalizedString(@"interactive_live_no_host_live");
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = LocalizedString(@"interactive_live_scenes");
    }
    return _titleLabel;
}
    ...
    [LiveRTSManager liveClearUserWithBlock:^(RTSACKModel *_Nonnull model) {
        [LiveRTSManager liveGetActiveLiveRoomListWithBlock:^(NSArray<LiveRoomInfoModel *> *roomList, RTSACKModel *model) {
            ...
            for (int i = 0; i < roomList.count; i++) {
                LiveRoomInfoModel *roomInfoModel = roomList[i];
                if (![ReportComponent containsBlockedKey:roomInfoModel.roomID]) {
                    [list addObject:roomInfoModel];
                }
            }
            roomList = [list copy];
