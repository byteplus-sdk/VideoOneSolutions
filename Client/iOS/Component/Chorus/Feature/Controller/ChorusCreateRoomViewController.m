...
- (void)addSubviewAndConstraints {
    [self.view insertSubview:self.bgImageView belowSubview:self.navView];
    ...
    [self.roomTitleView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.navView.mas_bottom).offset(18);
    }];
...
- (UILabel *)roomNameLabel {
    if (!_roomNameLabel) {
        ...
        _roomNameLabel.text = [NSString stringWithFormat:LocalizedString(@"label_room_title_%@"), [LocalUserComponent userModel].name];
    }
...
- (UIButton *)joinButton {
    if (!_joinButton) {
        ...
        [_joinButton setTitle:LocalizedString(@"button_start_name") forState:UIControlStateNormal];
...
- (ChorusCreateRoomTipView *)tipView {
    if (!_tipView) {
        ...
        _tipView.message = LocalizedString(@"label_create_tip");
    }
    return _tipView;
}
