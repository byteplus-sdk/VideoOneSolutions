...
+ (ChorusMusicLyricModel *)analyzeLrcByPath:(NSString *)path
    ...
    NSMutableArray<ChorusMusicLyricLineModel *> *lrcLines = [NSMutableArray array];
    ...
    [jsonArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        ChorusMusicLyricLineModel *line = nil;
        if (lrcFormat == ChorusMusicLrcFormatKRC) {
            line = [self analyzeLineLrcKRC:obj];
        } else if (lrcFormat == ChorusMusicLrcFormatLRC) {
    ...
    ChorusMusicLyricModel *infoModel = [[ChorusMusicLyricModel alloc] init];
...
/*
  ...
  */
+ (ChorusMusicLyricLineModel *)analyzeLineLrcKRC:(NSString *)lrcLine {
    ...
    NSMutableArray <ChorusMusicLyricWordModel *> *wordModelArray = [[NSMutableArray alloc] init];
    ...
    for (int i = 0; i < timList.count; i++) {
        ChorusMusicLyricWordModel *wordModel = [[ChorusMusicLyricWordModel alloc] init];
    ...
    ChorusMusicLyricLineModel *lyricLineModel = [[ChorusMusicLyricLineModel alloc] init];
 ...
 */
+ (ChorusMusicLyricLineModel *)analyzeLineLrcLRC:(NSString *)lrcLine {
    if (!lrcLine || lrcLine.length <= 0) {
        return nil;
    }
