...
@implementation ChorusMusicLyricModel

@end

@implementation ChorusMusicLyricLineModel
...
- (CGFloat)calculateCurrentLrcProgress:(NSInteger)currentTime {
    ...
    ChorusMusicLyricWordModel *lastWordModel;
    ...
    if (offsetTime < allWordDuration) {
        for (int i = 0; i < self.words.count; i++) {
            ChorusMusicLyricWordModel *wordModel = self.words[i];
            if (offsetTime >= wordModel.offset && offsetTime <= wordModel.offset + wordModel.duration) {
                ...
                ChorusMusicLyricWordModel *nextModel = self.words[i + 1];
...
@implementation ChorusMusicLyricWordModel
