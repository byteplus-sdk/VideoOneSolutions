...
#import "ChorusMusicLyricLabel.h"

@interface ChorusMusicLyricCell ()

@property (nonatomic, strong) ChorusMusicLyricLabel *messageLabel;
...
@implementation ChorusMusicLyricCell
...
- (void)layoutSubviews {
    ...
    BOOL isKRC = self.config.lrcFormat == ChorusMusicLrcFormatKRC;
...
- (void)setLineModel:(ChorusMusicLyricLineModel *)lineModel {
    _lineModel = lineModel;
    [self.messageLabel setText:lineModel.content];
}

- (void)setConfig:(ChorusMusicLyricConfig *)config {
    _config = config;
...
#pragma mark - Getter
- (ChorusMusicLyricLabel *)messageLabel {
  if (!_messageLabel) {
      _messageLabel = [[ChorusMusicLyricLabel alloc] init];
