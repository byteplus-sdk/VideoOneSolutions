...
@interface KTVMusicLyricView () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) KTVMusicLyricModel *model;
...
@implementation KTVMusicLyricView
...
- (void)loadLrcByPath:(NSString *)path
    ...
    self.model = [KTVMusicLyricsAnalyzer analyzeLrcByPath:path
...
- (void)setCurrentRow:(NSInteger)currentRow currentTime:(NSInteger)currentTime {
    if (_currentRow != currentRow) {
        ...
        KTVMusicLyricCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
...
- (void)updateLyric:(NSInteger)progress {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        KTVMusicLyricLineModel *lastLineModel = self.model.lines.lastObject;
        ...
        for (int i = 0; i < self.model.lines.count; i++) {
            KTVMusicLyricLineModel *lineModel = self.model.lines[i];
...
- (void)linefeed:(NSInteger)progress {
    ...
    KTVMusicLyricLineModel *lineModel = self.model.lines[pos];
...
- (NSInteger)getCurPos:(NSInteger)progress {
    ...
    for (int i = 0; i < self.model.lines.count; i++) {
        KTVMusicLyricLineModel *lineModel = self.model.lines[i];
        KTVMusicLyricWordModel *lastWordModel = lineModel.words.lastObject;
        if (lastWordModel) {
...
- (BOOL)isLineFeedAtPos:(NSInteger)pos {
    ...
    for (int i = 0; i < self.posList.count; i++) {
        KTVMusicLyricPosModel *posModel = self.posList[i];
        if (posModel.pos == pos) {
            return NO;
        }
...
- (void)addPos:(NSInteger)pos {
    KTVMusicLyricPosModel *posModel = [[KTVMusicLyricPosModel alloc] init];
    posModel.pos = pos;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    KTVMusicLyricCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(KTVMusicLyricCell.class)];
    KTVMusicLyricLineModel *lineModel = self.model.lines[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [KTVMusicLyricCell getCellHeight:self.model.lines[indexPath.row]
                                     config:self.config
                                   maxWidth:self.width];
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[KTVMusicLyricCell class] forCellReuseIdentifier:NSStringFromClass(KTVMusicLyricCell.class)];
