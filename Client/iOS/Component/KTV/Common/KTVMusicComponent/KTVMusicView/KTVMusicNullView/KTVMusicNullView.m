...
- (void)setLoginUserModel:(KTVUserModel *)loginUserModel {
    ...
    if (loginUserModel.status == KTVUserStatusActive ||
        ...
        self.titleLabel.text = LocalizedString(@"label_no_singing_host_tip");
...
- (void)buttonAction {
    if (self.loginUserModel.status == KTVUserStatusActive ||
        self.loginUserModel.userRole == KTVUserRoleHost) {
