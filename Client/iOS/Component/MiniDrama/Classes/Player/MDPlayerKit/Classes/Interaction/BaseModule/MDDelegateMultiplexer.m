...
static void __MDDelegateMultiplexer_IS_CALLING_OUT_TO_A_DELEGATE__(NSInvocation *invocation, id target) {
    [invocation invokeWithTarget:target];
}

@implementation MDDelegateMultiplexer {
...
- (void)forwardInvocation:(NSInvocation *)anInvocation {
    ...
    for (id delegate in delegates) {
        if ([delegate respondsToSelector:sel]) {
            __MDDelegateMultiplexer_IS_CALLING_OUT_TO_A_DELEGATE__(anInvocation, delegate);
        }
    }
}
