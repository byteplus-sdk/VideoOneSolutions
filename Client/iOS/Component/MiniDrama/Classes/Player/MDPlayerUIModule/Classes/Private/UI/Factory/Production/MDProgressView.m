...
@interface MDProgressView () <MDProgressSliderDelegate>
...
@property (nonatomic, strong) MDProgressSlider *progressSlider;

@property (nonatomic, copy) void (^valueChanged)(CGFloat);

@property (nonatomic, assign) BOOL isManualChanged;
...
- (void)registEvents {
    [[MDEventMessageBus universalBus] registEvent:MDPlayEventProgressValueIncrease withAction:@selector(sliderValueIncrease:) ofTarget:self];
}
...
- (void)sliderValueIncrease:(id)param {
    if ([param isKindOfClass:[NSDictionary class]]) {
        ...
        if ([value isKindOfClass:[NSNumber class]]) {
            ...
            [self progressManualChanged:self.progressSlider.progressValue touchEnd:NO];
        }
    }
}
...
- (MDProgressSlider *)progressSlider {
    if (!_progressSlider) {
        _progressSlider = [MDProgressSlider new];
...
- (void)elementViewAction {
    self.valueChanged = ^(CGFloat sliderValue) {
        NSTimeInterval destination = [[MDEventPoster currentPoster] duration] * sliderValue * 1.000;
        [[MDEventMessageBus universalBus] postEvent:MDPlayEventSeek withObject:@(destination) rightNow:YES];
    };
}
