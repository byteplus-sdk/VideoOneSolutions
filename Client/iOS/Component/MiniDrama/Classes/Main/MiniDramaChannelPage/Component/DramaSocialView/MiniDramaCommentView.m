...
@interface MiniDramaCommentView ()<UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate>
...
@implementation MiniDramaCommentView
...
- (void)showCommentTip {
    ...
    actionModel.title = LocalizedStringFromBundle(@"tip_close", @"VodPlayer");
    [[AlertActionManager shareAlertActionManager] showWithTitle:LocalizedStringFromBundle(@"tip_title", @"VodPlayer")
                                                        message:LocalizedStringFromBundle(@"tip_comment_content", @"VodPlayer")
                                                        actions:@[actionModel]
                                                      hideDelay:0];
}
...
- (void)loadDataIfNeed {
    ...
    [NetworkingManager getDramaDataForVideoComment:self.videoModel.videoId 
...
- (void)updateTitle {
    self.titleLabel.text = [NSString stringWithFormat:@"%@ %@", [NSString stringForCount:self.commentModels.count], LocalizedStringFromBundle(@"comments", @"VodPlayer")];
}
...
- (void)deleteComment:(VECommentModel *)comment {
    ...
    [[AlertActionManager shareAlertActionManager] showWithTitle:LocalizedStringFromBundle(@"delete_comment_title", @"VodPlayer")
                                                        message:LocalizedStringFromBundle(@"delete_comment_content", @"VodPlayer")
...
- (void)createHeaderView {
    ...
    [tipButton setImage:[UIImage imageNamed:@"vod_tip_question_mark" bundleName:@"VodPlayer"] forState:UIControlStateNormal];
    ...
    if (self.axis == UILayoutConstraintAxisVertical) {
        ...
        [closeButton setImage:[UIImage imageNamed:@"vod_bar_close" bundleName:@"VodPlayer"] forState:UIControlStateNormal];
