//
//  MDSelectionBarView.m
//  VOLCDemo
//

#import "MDSelectionBarView.h"
#import <Masonry/Masonry.h>

static const NSUInteger MDSelectionBarViewTagOffset = 1024;

@interface MDSelectionBarView ()

@property (nonatomic) MDSelectionBarViewStyle style;
...
@implementation MDSelectionBarView

#pragma mark - Life Cycle

- (instancetype _Nonnull)initWithStyle:(MDSelectionBarViewStyle)style
    ...
    if (self) {
        ...
        if (MDSelectionBarViewStyleSelectedLine == style) {
...
- (void)updateConstraints {
    if (MDSelectionBarViewStyleSelectedLine == self.style) {
...
- (void)updateWithTitles:(NSArray<NSString *> * _Nonnull)titles selectedIndex:(NSUInteger)selectedIndex {
    ...
    for (NSString *title in titles) {
        ...
        UIButton *button = [self buttonWithTitle:title tag:MDSelectionBarViewTagOffset + index++];
...
- (void)titleButtonIsSelected:(UIButton *)sender {
    ...
    !self.selectionCallback ?: self.selectionCallback(self, sender.tag - MDSelectionBarViewTagOffset);
}
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated {
    if (self.selectedIndex != selectedIndex) {
        for (UIButton *button in self.buttons) {
            [self configureButton:button selected:(button.tag - MDSelectionBarViewTagOffset == selectedIndex)];
        }
        if (MDSelectionBarViewStyleSelectedLine == self.style) {
            if (animated) {
                [self toggleAnimation];
...
- (NSUInteger)selectedIndex {
    for (UIButton *button in self.buttons) {
        if (button.selected) {
            return button.tag - MDSelectionBarViewTagOffset;
        }
    }
