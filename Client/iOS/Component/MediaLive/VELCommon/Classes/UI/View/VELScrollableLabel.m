...
@interface VELScrollableLabel ()

@property (nonatomic, assign) CGSize oldSize;

@end

@implementation VELScrollableLabel
...
//        [self.label.layer removeAllAnimations];
        [self.label.layer addAnimation:[self vel_getLayerAnimation] forKey:ANIMATION_KEY];
    } else {
        [self.label.layer removeAllAnimations];
...
- (CAAnimation *)vel_getLayerAnimation {
    CGPoint anchorPoint = self.label.layer.anchorPoint;
    
    CGFloat t1 = 0.3;
    CGFloat t2 = [self vel_animationDuration:self.label.frame.size.width - self.frame.size.width];
    CGFloat t3 = 0.3;
    CGFloat t4 = 0;
...
- (CGFloat)vel_animationDuration:(CGFloat)length {
