...
- (void)setPlaceHolderColor:(UIColor *)placeHolderColor {
    ...
    self.textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:self.placeholderStr ?: @"" attributes:@{
        NSForegroundColorAttributeName: placeHolderColor ?: [UIColor clearColor],
...
- (void)cancelButtonAction {
    ...
    if (self.minLimit > 0) {
        ...
        if (IsEmptyStr(self.errorMessage)) {
            message = [NSString stringWithFormat:LocalizedStringFromBundle(@"user_name_rule", ToolKitBundleName), (long)self.maxLimit];
        } else {
