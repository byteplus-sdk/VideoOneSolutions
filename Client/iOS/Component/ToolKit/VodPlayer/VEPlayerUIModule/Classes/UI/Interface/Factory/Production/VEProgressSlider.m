...
- (void)seekingAction:(id)param {
    if ([param isKindOfClass:[NSDictionary class]]) {
        ...
        if ([value isKindOfClass:[NSNumber class]]) {
            self.thumbView.hidden = !((NSNumber *)value).boolValue;
        }
    }
}
...
- (UIImageView *)thumbView {
    if (!_thumbView) {
        ...
        _thumbView.image = [UIImage imageNamed:@"vod_slider_thumb" bundleName:@"VodPlayer"];
        _thumbView.contentMode = UIViewContentModeCenter;
...
- (void)setTouchDetective:(BOOL)touchDetective {
    ...
    [self.eventMessageBus postEvent:VEPlayProgressSliderGestureEnable withObject:@(touchDetective) rightNow:YES];
}
