...
- (void)showSpeedMenuAction:(id)param {
    ...
    self.selectionMenu.title = LocalizedStringFromBundle(@"play_back_speed", @"VodPlayer");
...
- (void)showResolutionMenuAction:(id)param {
    ...
    self.selectionMenu.title = LocalizedStringFromBundle(@"play_quality", @"VodPlayer");
...
- (void)showSmartSubtitleMenuAction:(id)param {
    ...
    id subTitleScene = self.scene;
    NSArray *subtitleList = [subTitleScene valueForKeyPath:@"manager.subtitleList"];
    NSInteger count = subtitleList.count;
    for (int i = 0; i< count; i++ ) {
        VEInterfaceDisplayItem *item = [VEInterfaceDisplayItem new];
        item.title = [[subtitleList objectAtIndex:i] valueForKey:@"languageName"];
        item.itemAction = VEPlayEventChangeSubtitle;
        item.actionParam = [subtitleList objectAtIndex:i];
        [subTitleItems addObject:item];
    }
    self.selectionMenu.title = LocalizedStringFromBundle(@"subtitle_default", @"VodPlayer");
