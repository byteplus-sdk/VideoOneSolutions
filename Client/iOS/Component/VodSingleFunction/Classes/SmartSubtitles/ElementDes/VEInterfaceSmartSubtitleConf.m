...
- (id)valueForUndefinedKey:(NSString *)key {
    VOLogE(VOVideoPlayback, @"valueForUndefinedKey: %@", key);
    [[ToastComponent shareToastComponent] showWithMessage:[NSString stringWithFormat:@"valueForUndefinedKey: %@", key]];
    return self.manager;
}
                ...
                if ([obj isKindOfClass:[SubtitleidType class]]) {
                    ...
                    if (subtitle.subtitleId == 0) {
                        ...
                        [weak_self.manager switchSubtitle:0 withBlock:^(BOOL result) {
                            [[ToastComponent shareToastComponent] showWithMessage:LocalizedStringFromBundle(@"subtitle_close", @"VodPlayer")];
                        }];
                    } else {
                        [button setImage:[UIImage imageNamed:@"subTitle" bundleName:@"VodPlayer"] forState:UIControlStateNormal];
                        [button setTitle:currentLanguageTitle forState:UIControlStateNormal];
                        if (weak_self.manager.currentSubtitleId == 0) {
                            [[ToastComponent shareToastComponent] showWithMessage:[NSString stringWithFormat:LocalizedStringFromBundle(@"subtitle_open", @"VodPlayer"), currentLanguageTitle]];
...
- (void)resetState {
    ...
    dispatch_queue_async_safe(dispatch_get_main_queue(), ^{
        [weak_self.button setImage:[UIImage imageNamed:@"subTitle" bundleName:@"VodPlayer"] forState:UIControlStateNormal];
        NSString *title = LocalizedStringFromBundle(@"subtitle_default", @"VodPlayer");
